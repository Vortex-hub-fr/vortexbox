function createAdminOpsRouteHandler(deps = {}) {
  ...
  return async function handleAdminOpsRoute(req, res, url) {
    if (req.method === "GET" && url.pathname === "/api/admin/railway-status") {
      ...
      if (!(requireAdminPermission ? requireAdminPermission(req, res, "ops:read") : requireAdminSession(req, res)))
        return true;
    ...
    if (req.method === "GET" && url.pathname === "/api/backup-site-zip") {
      ...
      if (!(requireAdminPermission ? requireAdminPermission(req, res, "backups:create") : requireAdminSession(req, res)))
        return true;
    ...
    if (req.method === "POST" && url.pathname === "/api/run-railway-update-terminal") {
      ...
      if (!(requireAdminPermission ? requireAdminPermission(req, res, "railway:update") : requireAdminSession(req, res)))
        return true;
