...
function isAuthAutoActivateOnEmailFailureEnabled() {
  return String(process.env.AUTH_AUTO_ACTIVATE_ON_EMAIL_FAILURE || "0") === "1";
}
...
function createUserAuthRouteHandler(deps = {}) {
  ...
  async function ensureEmailNotDeleted(email, res) {
    const normalized = String(email || "").trim().toLowerCase();
    if (!normalized || typeof isDeletedAuthEmail !== "function") return true;
    const deleted = await isDeletedAuthEmail(normalized);
    if (!deleted) return true;
    sendJson(res, 403, { ok: false, error: "Compte supprimé définitivement. Contactez l'administrateur." });
    return false;
  }
  ...
  async function handleAuthRequestActivation(req, res) {
    ...
    const signupAccessCode = String(body?.signupAccessCode || "").trim();
    ...
    if (!(await ensureEmailNotDeleted(email, res))) return;
    if (password.length < 6) {
      sendJson(res, 400, { ok: false, error: "Mot de passe trop court (6 caractères minimum)." });
      return;
    }
    const expectedSignupCode =
      typeof getSignupAccessCode === "function" ? String(await getSignupAccessCode()).trim() : "460460";
    if (!/^\d{6}$/.test(signupAccessCode) || signupAccessCode !== expectedSignupCode) {
      sendJson(res, 403, { ok: false, error: "Code de connexion VortexBox invalide." });
      return;
    }
    ...
    if (!sent.ok) {
      if (isAuthAutoActivateOnEmailFailureEnabled() && Number(sent.status || 0) >= 500) {
