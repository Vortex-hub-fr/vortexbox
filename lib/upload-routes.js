function createUploadRouteHandler(deps = {}) {
  ...
  return async function handleUploadRoute(req, res, url) {
    if (req.method === "POST" && url.pathname === "/api/upload") {
      ...
      if (!(requireAdminPermission ? requireAdminPermission(req, res, "uploads:write") : requireAdminSession(req, res)))
        return true;
    ...
    if (req.method === "GET" && url.pathname === "/api/uploads-list") {
      ...
      if (!(requireAdminPermission ? requireAdminPermission(req, res, "uploads:read") : requireAdminSession(req, res)))
        return true;
